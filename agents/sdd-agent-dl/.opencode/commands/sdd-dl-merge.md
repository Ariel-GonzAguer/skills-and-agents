...
3. Actualizar `CHANGELOG.md` con `.opencode/scripts/changelog.js --base <base>` al crear el cursor; luego usar el script sin argumentos.
4. Revisar y limpiar la redacción del changelog. Crear un commit solo si el usuario lo solicita explícitamente.
5. Preguntar antes de cambiar a la rama base, mergear y borrar la rama de feature.
6. Marcar la fase como completa en `specs/constitution/roadmap.md` y pasar a `state: merged`. Dejar estos cambios sin commit salvo solicitud explícita del usuario.
